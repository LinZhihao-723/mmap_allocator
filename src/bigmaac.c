...
#define OOM()                                                                  \
  ...
  errno = ENOMEM;
#define LARGER_GAP(h, a, b)                                                    \
  (((h->node_array[a]->size) > (h->node_array[b]->size)) ? a : b)
#define SIZE_TO_MULTIPLE(size, multiple)                                       \
  ((size % multiple) > 0 ? size + (multiple - size % multiple) : size)
...
static int remove_chunk_with_ptr(void *const ptr, void *const prev_ptr,
...
static void print_ll(node *head) {
  while (head != NULL) {
    fprintf(stderr, "%p n=%p, u=%d, p=%p, size=%ld, ptr=%p\n", head, head->next,
            head->in_use, head->previous, head->size, head->ptr);
    head = head->next;
  }
}
...
static void print_heap(heap *heap) {
  for (int i = 0; i < heap->used; i++) {
    fprintf(stderr, "parent %d node %d , ptr=%p size=%ld\n", (i - 1) / 2, i,
            heap->node_array[i]->ptr, heap->node_array[i]->size);
  }
}
...
static void heap_remove_idx(heap *const heap, const int idx) {
  ...
  heap->node_array[heap->used - 1]->heap_idx =
      idx; // node has moved up in the heap
...
static int heap_insert(node *const head, node *const n) {
  ...
  if (heap->used == heap->length) {
    heap->node_array = (node **)real_realloc(heap->node_array,
                                             sizeof(node *) * heap->length * 2);
...
static void bigmaac_init(void) {
  ...
  if (min_size_fry > min_size_bigmaac) {
    fprintf(stderr,
            "BigMaac: Failed to initialize library, fries must be smaller than "
            "bigmaac, %ld %ld\n",
  ...
  // reserve the full contiguous range
  base_fries = mmap(NULL, size_total, PROT_NONE, MAP_ANONYMOUS | MAP_PRIVATE,
                    -1, 0); 
  // At this point, the mapping is anonymous, meaning that no actual file
  // created to build this mapping yet.
  ...
  const int ret =
      mmap_tmpfile(base_fries, size_fries); // allocate fries right away
  if (ret < 0) {
...
static int mmap_tmpfile(void *const ptr, const size_t size) {
  char *const filename =
      (char *)real_malloc(sizeof(char) * (strlen(template) + 1));
  if (filename == NULL) {
  ...
  const int fd = mkstemp(filename); // Create a unique file.
  if (fd < 0) {
  ...
  ret = ftruncate(fd, size); // resize the file, and reserve enough space.
  if (ret != 0) {
    fprintf(stderr, "BigMaac: ftruncate failed! %s\n", strerror(errno));
    return -1;
  }
  // Ensure the mapping will be put on the given address, with MAP_FIXED tag.
  void *ret_ptr =
      mmap(ptr, size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, fd, 0);
  if (ret_ptr == MAP_FAILED) {
    fprintf(stderr,
            "BigMaac: mmap failed! mmap() [ active mmaps %d , bigmaac capacity "
            "free: %0.2f , fries capacity free: %0.2f, check "
            "/proc/sys/vm/max_map_count : %s\n",
            active_mmaps, 1.0 - ((float)used_fries) / size_fries,
            1.0 - ((float)used_bigmaacs) / size_bigmaac, strerror(errno));
    return -1;
  }
  active_mmaps++;

  // Close the file descriptor. This doesn't affect the mmap region.
  ret = close(fd);
  if (ret == -1) {
...
static void *create_chunk(size_t size) {
  node *const head = size > min_size_bigmaac
                         ? _head_bigmaacs
                         : _head_fries; // TODO lock per head?
...
static int remove_chunk_with_ptr(void *const ptr, void *const new_ptr,
  ...
  node *head = ptr < base_bigmaac ? _head_fries : _head_bigmaacs;
  if (head == _head_bigmaacs) {
    const void *remap = mmap(n->ptr, n->size, PROT_NONE,
...
void *realloc(void *ptr, size_t size) {
  ...
  if (ptr >= base_fries && ptr < end_bigmaac) {
    ...
    int r = remove_chunk_with_ptr(
        ptr, p,
        size); // Check if this pointer is>> address space reserved fr mmap
    if (r < 0) {
      OOM();
      return NULL;
    } else if (r == 0) {
  ...
  if (size > min_size_fry) {
    void *mallocd_p =
        real_realloc(ptr, size); // we have no idea of previous size
    if (mallocd_p == NULL) {
...
void free(void *ptr) {
  ...
  int chunks_removed = remove_chunk_with_ptr(
      ptr, NULL,
      0); // Check if this pointer is>> address space reserved fr mmap
  if (chunks_removed == 0) {
    fprintf(stderr,
            "BigMaac: Free was called on pointer that was not alloc'd %p\n",
            ptr);
    return;
  }
}
