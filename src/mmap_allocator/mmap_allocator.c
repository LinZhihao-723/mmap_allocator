...
bool LOCAL_HELPER config_parameters() {
    ...
    if (env_profile) {
        ...
        if (!profile_file) {
            fprintf(stderr, "Failed tp open the profile file in path: %s\n", env_profile);
            return false;
        }
    }
    ...
    if (env_profile_freq) {
        ...
        if (profile_frequency == 0) {
            fprintf(stderr, "Config error: %s cannot be zero.\n", env_mmap_allocator_min_bsize);
            return false;
        }
    }
...
LOCAL_HELPER void mmap_allocator_init() {
    ...
    if (!std_malloc || !std_free || !std_calloc || !std_realloc || !std_reallocarray) {
...
LOCAL_HELPER bool mmap_release(void* addr) {
    ...
    if (!heap_free(&mmap_heap, block)) {
        fprintf(stderr, "Failed to free as stdlib cannot allocate more memory for the heap.\n");
        GLOBAL_LOCK_RELEASE();
        return false;
    }
...
void* realloc(void* addr, size_t size) {
    ...
    if (addr >= mmap_region_base) {
        ...
        GLOBAL_LOCK_RELEASE();
        if (!block) {
            fprintf(stderr, "Realloc failed: cannot find the given address: %p.\n", addr);
            return NULL;
        }
